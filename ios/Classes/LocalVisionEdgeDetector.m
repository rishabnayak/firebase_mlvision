...
+ (void)handleDetection:(FIRVisionImage *)image options:(NSDictionary *)options result:(FlutterResult)result {
    FIRLocalModel *localModel = [[FIRModelManager modelManager] localModelWithName:options[@"dataset"]];
    if (localModel == nil) {
        ...
        [[FIRVision vision] onDeviceAutoMLImageLabelerWithOptions:[LocalVisionEdgeDetector parseOptions: options]];
        [labeler
         processImage:image
