...
+ (void)handleDetection:(FIRVisionImage *)image options:(NSDictionary *)options result:(FlutterResult)result {
    FIRRemoteModel *remoteModel = [[FIRModelManager modelManager] remoteModelWithName:options[@"dataset"]];
    if (remoteModel == nil) {
        FIRModelDownloadConditions *conditions =
        [[FIRModelDownloadConditions alloc] initWithAllowsCellularAccess:YES
                                             allowsBackgroundDownloading:YES];
        FIRRemoteModel *remoteModel =
        [[FIRRemoteModel alloc] initWithName:options[@"dataset"]
                          allowsModelUpdates:YES
                           initialConditions:conditions
                            updateConditions:conditions];
        [[FIRModelManager modelManager] registerRemoteModel:remoteModel];
        [[FIRModelManager modelManager] downloadRemoteModel:remoteModel];
        FIRVisionImageLabeler *labeler = [[FIRVision vision] onDeviceAutoMLImageLabelerWithOptions:[RemoteVisionEdgeDetector parseOptions: options]];
        [labeler
         processImage:image
        ...
        Boolean isModelDownloaded = [[FIRModelManager modelManager] isRemoteModelDownloaded:remoteModel];
...
+ (FIRVisionOnDeviceAutoMLImageLabelerOptions *)parseOptions:(NSDictionary *)optionsData {
    ...
    [[FIRVisionOnDeviceAutoMLImageLabelerOptions alloc]
     initWithRemoteModelName: dataset
     localModelName: nil];
